...
def CreateRoadSurfaces(inRoadsGDB, inSurfGDB, widthOption):
   ...
   - widthOption: Specifies which codeblock to use, with options MIN, MAX, or MEAN
   '''
   
   # Parameter check
   if widthOption not in ['MIN', 'MAX', 'MEAN']:
      printMsg('The buffer width options are "MIN", "MAX", or "MEAN". Aborting.')
      return
...
def main():
   ...
   inDir = r'H:\Enviva\Testing\TestSubset'
   inRoadsGDB = r'H:\Enviva\Testing\roads.gdb'
   meanSurfGDB = r'H:\Enviva\Roads\Outputs\roadSurfaces_mean.gdb'
   outFC = r'H:\Enviva\Roads\Outputs\roadSurfaces_merged.gdb\roadSurf_mean'
   
   # Set up function runs here
   #FilterAndProject(inDir, inRoadsGDB)
   #CreateRoadSurfaces(inRoadsGDB, minSurfGDB, "MIN")
   MergeFeatsInGDB(meanSurfGDB, outFC)
